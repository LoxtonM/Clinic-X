...
namespace ClinicX.Meta
{
    ...
    public class DictatedLetterData : IDictatedLetterData
    {
        ...
        public DictatedLetterData(ClinicalContext cContext, ClinicXContext cXContext)
        {
            _clinContext = cContext;
            _clinicXContext = cXContext;
        }
               

        public List<DictatedLetter> GetDictatedLettersList(string staffcode)
        {
            IQueryable<DictatedLetter> letters = from l in _clinicXContext.DictatedLetters
        ...
        {
            DictatedLetter letter = _clinicXContext.DictatedLetters.FirstOrDefault(l => l.DoTID == dotID);
        ...
        {
            IQueryable<DictatedLettersPatient> patient = from p in _clinicXContext.DictatedLettersPatients
                          where p.DOTID == dotID
                          select p;
        ...
        {
            IQueryable<DictatedLettersCopy> copies = from c in _clinicXContext.DictatedLettersCopies
                       where c.DotID == dotID
                       select c;            
        ...
        {
            DictatedLettersCopy letter = _clinicXContext.DictatedLettersCopies.FirstOrDefault(x => x.CCID == id);
